...
public class product_functions implements Products {
    ...
    public String getAnylyzeFavouriteProduct(String id) {
        ...
        try {
            ...
            con = db.getConnection();
            String sql = " select order_item_id from transaction inner join customer on customer.customer_id = transaction.customer_id
            where transaction .customer_id=? Group by order_item_id order by count(order_item_id) desc limit 1;";
            PreparedStatement st = con.prepareStatement(sql);
            st.setString(1, id);
            ResultSet rs = st.executeQuery();
            while (rs.next()){
                Category category = new Category(rs.getString("order_item_id"));
                res=category.getProduct_name();
            }
           getOrder(res);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
